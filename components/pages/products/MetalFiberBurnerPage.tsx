...
const heroTags = ["표면연소", "적외선 복사열", "예혼합 연소", "균일 가열", "저NOx 대응"];
const applications = [
  "분체도장 오븐",
  "식품 가공 라인",
  "컨베이어 건조로",
...
const processFlow = ["가스·공기 예혼합", "다공성 연소면 통과", "표면연소 형성", "적외선 복사열 전달", "균일 공정 가열"];
const features = [
            ...
            <div className="flex flex-wrap items-center gap-2">
              {processFlow.map((step, index) => (
                <div key={step} className="flex items-center gap-2">
                  <div className={`px-3 py-2 text-[14px] tracking-[0.05em] text-center ${step === "표면연소 형성" || step === "적외선 복사열 전달" ? "bg-[#C05010] text-white" : step === "균일 공정 가열" ? "bg-[#2d2a28] text-white" : "bg-white border border-[#D4DAE2] text-[#5C6470]"}`}>{step}</div>
