...
const heroTags = ["오븐/건조 특화", "균일 열원", "중저온 공정", "공정 맞춤 제어"];
const applications = ["식품 가공", "플라스틱 성형", "도장 건조", "프린팅·코팅", "섬유 텐터 드라이어", "산업용 드라이 오븐"];
const processFlow = ["연료 공급", "버너 점화", "오븐 내 열풍 형성", "공정물 가열", "균일 온도 유지"];
const features = [
      ...
      <section className="px-6 md:px-12 py-16">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <div ref={heroRef} className={`transition-all duration-1000 ${heroInView ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"}`}>
            ...
            <h2 className="text-2xl md:text-3xl font-light tracking-[0.08em] text-[#2d2a28] mb-6">오븐 버너</h2>
            ...
            <p className="text-sm text-[#5C6470] leading-relaxed mb-8">
              오븐 버너는 단순 화력보다 공정 전체의 온도 균일성과 체류 시간 관리가 더 중요합니다. 엔비피코리아는
              오븐 크기, 제품 특성, 건조 시간, 목표 온도를 기준으로 열원 구성을 제안합니다.
            </p>
          ...
          <div className={`transition-all duration-1000 delay-300 ${heroInView ? "opacity-100 translate-x-0" : "opacity-0 translate-x-10"}`}>
            <div className="relative aspect-[4/3] overflow-hidden bg-white border border-[#C05010]/30">
              <Image src={`${S3}/images/burner/oven-burner-main.jpg`} alt="오븐 버너" fill className="object-cover" priority />
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {processFlow.map((step, index) => (
              <div key={step} className="flex items-center gap-2">
                <div className={`px-3 py-2 text-[14px] tracking-[0.05em] text-center ${step === "오븐 내 열풍 형성" || step === "공정물 가열" ? "bg-[#C05010] text-white" : step === "균일 온도 유지" ? "bg-[#2d2a28] text-white" : "bg-white border border-[#D4DAE2] text-[#5C6470]"}`}>{step}</div>
