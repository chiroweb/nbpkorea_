...
const heroTags = ["가스 제어 시스템", "안전 차단", "정밀 계장", "현장 맞춤 조합"];
const applications = ["산업용 버너", "가스히터", "오븐 및 건조기", "연소 제어반", "신규 설비 구축", "기존 설비 부품 교체"];
const processFlow = ["가스 유입", "필터 / 레귤레이터", "안전 차단", "유량 제어", "버너 공급"];
const features = [
      ...
      <section className="px-6 md:px-12 py-16">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <div ref={heroRef} className={`transition-all duration-1000 ${heroInView ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"}`}>
            ...
            <h2 className="text-2xl md:text-3xl font-light tracking-[0.08em] text-[#2d2a28] mb-6">밸브트레인 / 부품류</h2>
            ...
            <p className="text-sm text-[#5C6470] leading-relaxed mb-8">
              밸브트레인은 연소 시스템의 안전과 제어 품질을 좌우하는 핵심입니다. 엔비피코리아는 연료 조건,
              장비 규격, 배관 압력, 운전 인터록 조건을 기준으로 현장 맞춤형 가스 제어 구성을 제공합니다.
            </p>
          ...
          <div className={`transition-all duration-1000 delay-300 ${heroInView ? "opacity-100 translate-x-0" : "opacity-0 translate-x-10"}`}>
            <div className="relative aspect-[4/3] overflow-hidden bg-white border border-[#C05010]/30">
              <Image src={`${S3}/images/burner/valve-train-main.jpg`} alt="밸브트레인" fill className="object-cover" priority />
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {processFlow.map((step, index) => (
              <div key={step} className="flex items-center gap-2">
                <div className={`px-3 py-2 text-[14px] tracking-[0.05em] text-center ${step === "안전 차단" || step === "유량 제어" ? "bg-[#C05010] text-white" : step === "버너 공급" ? "bg-[#2d2a28] text-white" : "bg-white border border-[#D4DAE2] text-[#5C6470]"}`}>{step}</div>
